...
func UploadTransaction(csv_rows [][]string, account_id int64, db querier) error {
	transactions, err := parse_boa(csv_rows, account_id)
	if err != nil {
		return err
	}

	for _, v := range transactions {
		_, err = db.CreateTransaction(context.Background(), v)
		if err != nil {
			fmt.Println(err)
		}
	}
